    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    confidence_scores = pcls.select(dim=-1, index=0)

                    # Separate positive and negative samples
                    positive_sample_indices = torch.where(cls_labels == 0)[0]
                    negative_sample_indices = torch.where(cls_labels != 0)[0]

                    # Handle positive samples (person class)
                    if positive_sample_indices.numel() > 0:
                        pos_confidences = confidence_scores[positive_sample_indices]
                        pos_targets = torch.full_like(pos_confidences, self.cp)
                        positive_loss = self.BCEcls(pos_confidences, pos_targets)
                        lcls += positive_loss

                    # Handle negative samples 
                    if negative_sample_indices.numel() > 0:
                        neg_confidences = confidence_scores[negative_sample_indices]
                        neg_targets = torch.full_like(neg_confidences, self.cn)
                        negative_loss = self.BCEcls(neg_confidences, neg_targets)
                        lcls += negative_loss
